...
import DirectMessages from "../models/directMesaages.js";
export async function getDirectMessageList(id) {
...
export async function getUserDirectMessages(id, recipient) {
    const user = await User.findOne({ _id: id });
    if (!user) {
        throw new Error('User not found');
    }
...
;
export async function addDirectMessage(id, recipient) {
    const user = await User.findOne({ _id: id });
    if (!user) {
        throw new Error('User not found');
    }
...
export async function sendMessage(id, recipient, contentType, content) {
    const user = await User.findOne({ _id: id });
    if (!user) {
        throw new Error('User not found');
    }
...
//# sourceMappingURL=manageDirectManages.js.map