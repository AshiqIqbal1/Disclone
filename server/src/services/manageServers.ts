...
import { Invitation } from "../models/directMesaages.js";
...
export async function getServerInformation(serverid: string) {
...
export async function getServerList(userid: string) {
...
export async function textChannelSendMessage(
    serverid: string, 
    channelid: string, 
    userid: string,
...
export async function joinFriendServer(
    userid: string,
    serverid:string
) {
    const user = await User.findOne({ _id: userid });
